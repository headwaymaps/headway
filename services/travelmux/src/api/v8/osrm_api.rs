...
impl From<Itinerary> for Route {
    fn from(itinerary: Itinerary) -> Self {
        Route {
            ...
            legs: itinerary.legs.into_iter().map(RouteLeg::from).collect(),
        }
    }
}
...
impl RouteStep {
    fn from_maneuver(
        ...
        let banner_instructions = VisualInstructionBanner::from_maneuver(&maneuver, next_maneuver);
...
impl VisualInstructionBanner {
    fn from_maneuver(maneuver: &Maneuver, next_maneuver: Option<&Maneuver>) -> Option<Vec<Self>> {
        ...
        let instruction = VisualInstructionBanner {
            distance_along_geometry: maneuver.distance_meters,
            primary,
        };
