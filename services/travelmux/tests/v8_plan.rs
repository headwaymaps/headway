...
mod integration_tests {
    ...
    static SERVER_ROOT: &str = "http://localhost:8000/v8";
    ...
    #[test]
    fn get_transit_plan() {
        let body = get_plan("fromPlace=-122.339414%2C47.575837&toPlace=-122.347234%2C47.651048&numItineraries=3&mode=TRANSIT");
    ...
    #[test]
    fn bad_mode() {
        let url = format!("{SERVER_ROOT}/plan?fromPlace=-122.339414%2C47.575837&toPlace=-122.347234%2C47.651048&numItineraries=3&mode=FAKE_MODE");
        let response = reqwest::blocking::get(url).unwrap();
    ...
    #[test]
    fn bad_date_time() {
        let url = format!("{SERVER_ROOT}/plan?fromPlace=-122.339414%2C47.575837&toPlace=-122.347234%2C47.651048&numItineraries=3&mode=TRANSIT&dateTime=noon");
        let response = reqwest::blocking::get(url).unwrap();
        assert!(!response.status().is_success());
    ...
    fn get_local_walk_plan() {
        let body = get_plan("fromPlace=-122.339414%2C47.575837&toPlace=-122.347234%2C47.651048&numItineraries=2&mode=WALK");
