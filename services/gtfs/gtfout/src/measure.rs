...
pub fn measure(
    ...
    let auth = feed.authorization.as_ref().map(|auth| &auth.kind);
    let secret = auth.as_ref().and_then(|_| config.get(&feed.id));

    let body = match fetch_feed(client, url, &feed.id, auth, config) {
        Ok(body) => body,
        Err(e) => {
            return Outcome {
...
pub fn verify_feed(
    ...
    authenticated_request(client, url, &feed.id, auth, config)
        .and_then(|request| {
            Ok(request
                .send()
                .map_err(reqwest::Error::without_url)?
...
fn authenticated_request(
    ...
    if let Some(auth) = auth {
        ...
        let credential = AuthKindSecret::resolve(auth, secret, feed_id)?;
        request = client.get(credential.url(url)).timeout(DOWNLOAD_TIMEOUT);
        for (name, value) in credential.headers().into_iter().flatten() {
...
fn bbox_from_gtfs_zip(zip_bytes: &[u8]) -> Result<Rect> {
...
fn bbox_from_stops_csv(contents: &str) -> Result<Rect> {
