...
use crate::api_keys::{api_key_env_var, ApiKeys};
use crate::dmfr::Feed;
...
pub fn measure(
    ...
    match fetch_and_measure(client, feed, url, api_keys) {
        Ok(bbox) => Measurement::from_bbox(bbox),
...
fn fetch_and_measure(
    ...
    let body = fetch_feed(client, url, &feed.id, auth.as_ref(), api_keys)?;
    bbox_from_gtfs_zip(&body)
}
...
pub fn fetch_feed(
    ...
    if let Some(auth) = auth {
        let Some(key) = api_keys.get(feed_id) else {
            return Err(format!(
                "needs an API key; set {} to fetch it",
                api_key_env_var(feed_id)
            )
            .into());
        };
...
pub fn bbox_from_stops_csv(contents: &str) -> Result<Rect> {
    ...
    bbox.ok_or_else(|| "stops.txt has no usable coordinates".into())
}
...
mod tests {
    ...
    fn bbox_from_stops() {
        ...
        assert_eq!(bbox.min(), Point::new(-122.3, 47.6));
        assert_eq!(bbox.max(), Point::new(-122.1, 47.7));
    }
