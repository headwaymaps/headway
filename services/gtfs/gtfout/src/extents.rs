...
/// Where the index lives unless a tool is pointed somewhere else.
pub const DEFAULT_PATH: &str = "data/transit-zoner/feed-extents.gpkg";
...
/// A static feed a zone can be built from: measured, and described by the
/// atlas. What [`FeedExtents::measured_feeds`] hands back.
#[derive(Debug, Clone)]
pub struct MeasuredFeed {
    pub feed: ZoneFeed,
    pub extent: Rect,
}
...
impl FeedExtents {
    ...
    pub fn measured_keys(&self) -> Result<HashMap<(FeedId, String), bool>> {
        ...
        for feature in layer.features()? {
            ...
            keys.insert((FeedId::from(feed_id), url), succeeded);
        }
    ...
    fn forget(&self, feed_ids: &[FeedId]) -> Result<usize> {
    ...
    /// those feeds.
    pub fn insert(&self, measurements: &[(FeedId, String, Measurement)]) -> Result<()> {
        let feed_ids: Vec<FeedId> = measurements
            .iter()
    ...
    pub fn set_metadata(&self, metadata: &[(FeedId, ZoneFeed)]) -> Result<usize> {
    ...
    pub fn measured_feeds(&self) -> Result<Vec<MeasuredFeed>> {
        let layer = self.gpkg.layer(LAYER)?;
        ...
        for feature in layer.select("error IS NULL AND metadata IS NOT NULL", &[])? {
            ...
            feeds.push(MeasuredFeed {
                feed,
                extent: Rect::new(coord! { x: min_x, y: min_y }, coord! { x: max_x, y: max_y }),
            });
        }
    ...
    pub fn feeds_in(&self, area: &Rect) -> Result<Vec<FeedId>> {
        let layer = self.gpkg.layer(LAYER)?;
        ...
        for feature in layer.features_in(bbox)? {
            ...
            if let Some(feed_id) = feature.value("feed_id").and_then(text) {
                feed_ids.push(FeedId::from(feed_id));
            }
        }
    ...
    pub fn failures(&self) -> Result<Vec<(FeedId, String, String)>> {
        let layer = self.gpkg.layer(LAYER)?;
        ...
        for feature in layer.select("error IS NOT NULL", &[])? {
            ...
            failures.push((FeedId::from(feed_id), url, error));
        }
...
mod tests {
    ...
    fn described(feed_id: &str, provider: &str) -> (FeedId, ZoneFeed) {
        (
            FeedId::from(feed_id),
            ZoneFeed {
                feed_onestop_id: feed_id.into(),
    ...
    fn extents_by_feed_id(extents: &FeedExtents) -> HashMap<FeedId, Rect> {
        extents
            .measured_feeds()
            .unwrap()
            .into_iter()
            .map(|measured| (measured.feed.feed_onestop_id, measured.extent))
            .collect()
    }
    ...
    fn re_measuring_a_feed_updates_its_record_rather_than_duplicating_it() {
        ...
        let feed = FeedId::from("f-c23-soundtransit");
        ...
        extents
            .set_metadata(&[described(feed.as_str(), "Sound Transit")])
            .unwrap();
    ...
    fn the_atlas_details_survive_the_round_trip() {
        ...
        feed.realtime.push(crate::transit_zone::zone::ZoneRealtime {
            ...
            urls: crate::atlas::dmfr::RealtimeUrls {
                trip_updates: Some("https://example.com/tu.pb".to_owned()),
                ..Default::default()
            },
            authorization: Some(crate::atlas::dmfr::Authorization {
        ...
        let MeasuredFeed { feed, extent } = &feeds[0];
    ...
    fn re_describing_a_feed_replaces_what_the_atlas_used_to_say() {
        ...
        let feeds = extents.measured_feeds().unwrap();
        assert_eq!(feeds.len(), 1);
        assert_eq!(feeds[0].feed.provider, "New Name");
        assert_eq!(feeds[0].feed.url, "https://example.com/new.zip");
    }
    ...
    fn forgetting_one_feed_leaves_the_others_alone() {
        ...
        assert_eq!(extents.forget(&["f-a".into()]).unwrap(), 1);
        assert_eq!(
            extents
                .feeds_in(&rect(-122.462, 47.394, -122.005, 47.831))
                .unwrap(),
