...
impl Geohash {
    ...
    /// notably excludes `a`, `i`, `l` and `o` - the reason a name component
    /// like "bay~area" can't be mistaken for one).
    pub fn parse(s: &str) -> Option<Self> {
        if s.is_empty() || s.len() > Self::MAX_LEN {
            return None;
        }
        // Decoding is the alphabet check: the crate rejects anything it can't
        // read, and we'd rather ask it than keep our own copy of the table.
        geohash::decode_bbox(s).ok()?;
    ...
    pub fn cell(&self) -> Rect {
        geohash::decode_bbox(&self.0).expect("parse() only accepts decodable geohashes")
    }
    ...
    pub fn cell_with_margin(&self) -> Rect {
        ...
        Rect::new(
            coord! { x: cell.min().x - width, y: cell.min().y - height },
            coord! { x: cell.max().x + width, y: cell.max().y + height },
        )
    }
}
