...
use gtfout::measure;
use gtfout::transit_zone::feed_info::rewrite_feed_info;
...
fn main() -> Result<()> {
    ...
    for feed in &zone.feeds {
        let feed_id = &feed.feed_onestop_id;
...
fn feeds_missing_credentials<'a>(zone: &'a Zone, credentials: &FeedConfig) -> Vec<&'a ZoneFeed> {
    zone.feeds
        .iter()
        .filter(|feed| {
            feed.authorization.is_some() && !credentials.has_credential(&feed.feed_onestop_id)
        })
        .collect()
}
...
fn repack(zip_bytes: &[u8], feed_id: &FeedId, output_path: &Path) -> Result<()> {
...
mod tests {
    ...
    fn config(pairs: &[(&str, &str)]) -> FeedConfig {
        pairs
            .iter()
            .map(|(k, v)| (gtfout::atlas::dmfr::FeedId::from(*k), (*v).to_owned()))
            .collect()
    }
