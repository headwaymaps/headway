...
use gtfout::atlas::dmfr::{self, FeedCore, FeedId, StaticFeed};
use gtfout::atlas::AtlasArgs;
use gtfout::extents::{self, FeedExtents};
use gtfout::feed_config::{self, FeedConfig};
use gtfout::measure::{self, Measurement};
use gtfout::transit_zone::zone::ZoneFeed;
use gtfout::Result;
...
The atlas clone is read as it stands; pass --download to refresh it first, which
is how feeds newly catalogued upstream reach the index.
...
Examples:
  ...
  build-gtfs-index --download             refresh the atlas, then update
...
struct Args {
    ...
    #[arg(long, default_value = extents::DEFAULT_PATH)]
    out: PathBuf,
...
fn main() -> Result<()> {
    ...
    let to_measure: Vec<(FeedCore, String)> = selected
        .into_iter()
        .map(|feed| (feed.feed.clone(), feed.url.clone()))
        .collect();
    if args.feeds.is_empty() {
        eprintln!("{} fetchable gtfs feeds in the atlas", to_measure.len());
    } else {
        eprintln!("{} feed(s) named", to_measure.len());
    }
    ...
    let mut retries = 0;
    let outstanding: Vec<(FeedCore, String)> = if args.feeds.is_empty() {
    ...
    let describe_measured = || -> Result<()> {
        let described: Vec<(FeedId, ZoneFeed)> = catalog
            .static_feeds
            .iter()
            .map(|feed| (feed.id().to_owned(), ZoneFeed::from(feed)))
            .collect();
        eprintln!(
...
fn select_feeds<'a>(catalog: &'a dmfr::Catalog, wanted: &[FeedId]) -> Result<Vec<&'a StaticFeed>> {
    if wanted.is_empty() {
        return Ok(catalog.static_feeds.iter().collect());
    }

    let by_id: HashMap<&FeedId, &StaticFeed> = catalog
        .static_feeds
        .iter()
    ...
    if !unknown.is_empty() {
        let detail: Vec<String> = unknown
            .iter()
            .map(|id| match catalog.unsupported_reason(id) {
                Some(reason) => format!("{id} (in the atlas, but {reason})"),
...
fn write_config_template(
    ...
    let failed: std::collections::HashSet<FeedId> = extents
        .failures()?
        .into_iter()
        .map(|(feed_id, _, _)| feed_id)
        .collect();

    let needing: Vec<&FeedCore> = catalog
        .static_feeds
        .iter()
        .map(|gtfs| &gtfs.feed)
...
fn measure_all(
    to_measure: Vec<(FeedCore, String)>,
    extents: &FeedExtents,
    concurrency: usize,
    config: &FeedConfig,
) -> Result<()> {
    ...
    let (tx, rx) = mpsc::channel::<(FeedId, String, Measurement)>();
