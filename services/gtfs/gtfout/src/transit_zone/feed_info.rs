//! Stamping a feed's Onestop ID into its `feed_info.txt`, so the built
//! graph names feeds the same way the zone file does.

use crate::atlas::dmfr::FeedId;
use crate::Result;

/// Produces a `feed_info.txt` body carrying `feed_id`.
pub fn rewrite_feed_info(existing: Option<&str>, feed_id: &FeedId) -> Result<String> {
    ...
    set("feed_id", feed_id.to_string(), true);
...
mod tests {
    ...
    fn synthesizes_a_whole_file_when_there_is_none() {
        let out = rewrite_feed_info(None, &"f-c23-soundtransit".into()).unwrap();
        let fields = parse(&out);

        assert!(fields.contains(&("feed_id".to_owned(), "f-c23-soundtransit".to_owned())));
    ...
    fn overwrites_an_existing_feed_id_but_keeps_everything_else() {
        ...
        let out = rewrite_feed_info(Some(existing), &"f-sp-tmb".into()).unwrap();
        let fields = parse(&out);

        assert!(fields.contains(&("feed_id".to_owned(), "f-sp-tmb".to_owned())));
    ...
    fn adds_feed_id_when_the_agency_omitted_the_column() {
        ...
        let out = rewrite_feed_info(Some(existing), &"f-c28-wta".into()).unwrap();
        let fields = parse(&out);

        assert!(fields.contains(&("feed_id".to_owned(), "f-c28-wta".to_owned())));
    ...
    fn fills_in_only_the_required_fields_that_are_blank() {
        let existing = "feed_id,feed_publisher_name,feed_lang\n\
                        x,,de\n";
        let out = rewrite_feed_info(Some(existing), &"f-x".into()).unwrap();
        let fields = parse(&out);

        assert!(fields.contains(&(
            "feed_publisher_name".to_owned(),
            "Feed Publisher: f-x".to_owned()
        )));
    ...
    fn ignores_all_but_the_first_row_of_an_aggregate_feed() {
        let existing = "feed_id,feed_publisher_name,feed_lang\n\
                        a,First,en\n\
                        b,Second,fr\n";
        let out = rewrite_feed_info(Some(existing), &"f-agg".into()).unwrap();
        let fields = parse(&out);
    ...
    fn handles_a_header_only_feed_info() {
        let existing = "feed_publisher_name,feed_publisher_url,feed_lang\n";
        let out = rewrite_feed_info(Some(existing), &"f-empty".into()).unwrap();
        let fields = parse(&out);
        assert!(fields.contains(&("feed_id".to_owned(), "f-empty".to_owned())));
    }
}
