...
mod tests {
    ...
    fn single_char_middle_component_reads_as_a_geohash() {
        ...
        // hyphenated letter", so we take it as a geohash. Nothing locates a
        // feed by its geohash any more - that's what the measured index is for
        // - so this only decides where the name starts.
