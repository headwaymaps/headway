...
use geo::{coord, Rect};
use gtfout::atlas::dmfr::{AuthKind, FeedId, StreamKind};
use gtfout::extents::{FeedExtents, MeasuredFeed};
use gtfout::geom::RectExt;
use gtfout::transit_zone::zone::ZoneRealtime;
...
struct Args {
    ...
    #[arg(long, default_value = gtfout::extents::DEFAULT_PATH)]
    gtfs_index: PathBuf,
...
/// The page shows the kind as a label, so send its tag rather than the whole
/// tagged object.
fn auth_kind_tag<S: serde::Serializer>(
...
struct RealtimeSummary {
    ...
    #[serde(serialize_with = "auth_kind_tag")]
    authorization_type: Option<AuthKind>,
...
struct State {
    ...
    feeds: HashMap<FeedId, MeasuredFeed>,
}

fn realtime_summary(rt: &ZoneRealtime) -> RealtimeSummary {
    RealtimeSummary {
        feed_id: rt.feed_onestop_id.clone(),
        kinds: rt.urls.stream_kinds(),
        authorization_type: rt.authorization.as_ref().map(|a| a.kind.clone()),
...
fn summarize<'a>(
    state: &State,
    ids: impl IntoIterator<Item = &'a FeedId>,
    area: Option<&Rect>,
) -> Vec<FeedSummary> {
    ids.into_iter()
        .filter_map(|id| {
            ...
            Some(FeedSummary {
                ...
                url: feed.url.clone(),
                authorization_type: feed.authorization.as_ref().map(|a| a.kind.clone()),
...
async fn download_zone(state: web::Data<State>, request: web::Json<ZoneRequest>) -> impl Responder {
    ...
    let zone = gtfout::transit_zone::Zone::new(&area, feeds);
...
async fn main() -> std::io::Result<()> {
    ...
    if !args.gtfs_index.is_file() {
        let path = args.gtfs_index.display();
        eprintln!("no feed-extents index at {path}");
        eprintln!("Build one with:");
        eprintln!("    bin/build-gtfs-index --out {path}");
        eprintln!("That clones the transitland atlas and measures every feed in it, so the");
        eprintln!("first run takes a while.");
        std::process::exit(1);
    }
    ...
    let feeds: HashMap<FeedId, MeasuredFeed> = extents
        .measured_feeds()
        .unwrap_or_else(|e| {
        ...
        .into_iter()
        .map(|measured| (measured.feed.feed_onestop_id.clone(), measured))
        .collect();
